...
if __name__ == '__main__':
    ...
    label = train_label[0]
    att_Unet_test = torch.unsqueeze(torch.Tensor(numpy.load(glob(os.path.join("validate_rainfall_pred_att","*"))[0])),dim=0)
    Unet_test = torch.unsqueeze(torch.Tensor(numpy.load(glob(os.path.join("validate_rainfall_pred","*"))[0])),dim=0)
########################################################绘图##################################################################################################################
    ...
    #
    # plt.savefig("pred_rainfall_validate_0.png")
    # plt.show()
####################################################################################################################################################################################
    c1 = nn.L1Loss()
    c2 = nn.MSELoss()
