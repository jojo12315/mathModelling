...
if __name__ == '__main__':
    ...
    label = train_label[3]
    att_Unet_test = torch.unsqueeze(torch.Tensor(numpy.load(glob(os.path.join("test_rainfall_pred_att","*"))[3])),dim=0)
    Unet_test = torch.unsqueeze(torch.Tensor(numpy.load(glob(os.path.join("test_rainfall_pred","*"))[3])),dim=0)
########################################################绘图##################################################################################################################
    ...
    #
    # plt.savefig("pred_rainfall_test_3.png")
    # plt.show()
####################################################################################################################################################################################
    c1 = nn.L1Loss()
    c2 = nn.MSELoss()
