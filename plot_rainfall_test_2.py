...
if __name__ == '__main__':
    ...
    label = train_label[2]
    att_Unet_test = torch.unsqueeze(torch.Tensor(numpy.load(glob(os.path.join("test_rainfall_pred_att","*"))[2])),dim=0)
    Unet_test = torch.unsqueeze(torch.Tensor(numpy.load(glob(os.path.join("test_rainfall_pred","*"))[2])),dim=0)
########################################################绘图##################################################################################################################
    ...
    #
    # plt.savefig("pred_rainfall_test_2.png")
    # plt.show()
####################################################################################################################################################################################
    c1 = nn.L1Loss()
    c2 = nn.MSELoss()
