...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, bath_normal=False):
        ...
        layers = [
            ...
            # stride：步长
            # padding：边缘填充
            nn.Conv3d(in_channels, channels, kernel_size=(1,3,3), stride=1, padding=0),
            nn.ReLU(True),

            nn.Conv3d(channels, out_channels, kernel_size=(1,3,3), stride=1, padding=0),
            nn.ReLU(True)
        ]
...
class UpSampling(nn.Module):
    def __init__(self, in_channels, out_channels, batch_normal=False, bilinear=False):
        ...
        translayers = [
            nn.ConvTranspose3d(in_channels, int(in_channels/2), kernel_size=(1, 3, 3), stride=1, padding=0),
            nn.ConvTranspose3d(int(in_channels/2), int(in_channels/4), kernel_size=(1, 3, 3), stride=1, padding=0),
            nn.ConvTranspose3d(int(in_channels/4), in_channels, kernel_size=2, stride=2)
        ]
        if bilinear:
            # 采用双线性插值的方法进行上采样
            ...
            # 采用反卷积进行上采样
            self.up = nn.Sequential(*translayers)
        self.conv = nn.Conv3d(int(in_channels + in_channels / 2), out_channels, kernel_size=1)
        self.re_W = nn.ConvTranspose3d(in_channels=in_channels, out_channels=in_channels, kernel_size=(1,2,2), stride=1)
        self.re_D = nn.ConvTranspose3d(in_channels=in_channels, out_channels=in_channels, kernel_size=(2,1,1),stride=1)
    ...
    def forward(self, inputs1, inputs2):
        ...
        if inputs1.shape[3] != inputs2.shape[3]:
            inputs1 = self.re_W(inputs1)
        if inputs1.shape[2] != inputs2.shape[2]:
            inputs1 = self.re_D(inputs1)
        # 进行特征融合
...
class LastConv(nn.Module):
    def __init__(self, in_channels, out_channels ):
        ...
        translayers = [
            nn.ConvTranspose3d(in_channels, int(in_channels / 2), kernel_size=(1, 3, 3), stride=1, padding=0),
            nn.ConvTranspose3d(int(in_channels / 2), in_channels, kernel_size=(1, 3, 3), stride=1, padding=0),
            nn.Conv3d(in_channels, out_channels, kernel_size=1)
        ]
        self.conv = nn.Sequential(*translayers)

    def forward(self, x):
        return self.conv(x)

class UNet3D(nn.Module):
    def __init__(self, in_channels, num_classes=1, batch_normal=False, bilinear=False):
        ...
        self.up_3 = UpSampling(64, 32, self.batch_normal, self.bilinear)
