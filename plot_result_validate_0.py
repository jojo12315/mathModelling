...
if __name__ == '__main__':
    ...
    att_ConvLSTM_test = torch.squeeze(torch.Tensor(numpy.load(glob(os.path.join("att_ConvLSTM_pred_validate","*"))[0])),dim=0)

    Conv_LSTM_test = torch.squeeze(torch.Tensor(numpy.load(glob(os.path.join("ConvLSTM_pred_validate","*"))[0])),dim=0)

    Unet3D_test = torch.squeeze(torch.Tensor(numpy.load(glob(os.path.join("Unet3D_pred","*"))[0])).permute(0, 2, 1, 3, 4),dim=0)
    ...
    # plt.savefig("pred_result_validate_0.png")
    # plt.show()
